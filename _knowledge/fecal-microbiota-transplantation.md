---
title: Fecal microbiota transplantation (FMT)
category: treatments
summary: Transferring stool from a screened donor to a patient to restore a healthier gut microbial community.
tags: [FMT, microbiota transplantation, C. difficile]
evidence: established
status: draft
last_reviewed: 2026-09-25
key_points:
  - Plain-language point one.
  - Plain-language point two.
sources:
  - title: PubMed search — fecal microbiota transplantation
    url: https://pubmed.ncbi.nlm.nih.gov/?term=fecal+microbiota+transplantation
  - title: ClinicalTrials.gov search — fecal microbiota transplantation
    url: https://clinicaltrials.gov/search?term=fecal%20microbiota%20transplantation
---

> **Example entry — outline only.** Fill in each section with a summary of the evidence and cite the studies you draw on in the `sources` list above.

## What it is

## How it is delivered

## Where the evidence is strongest

## Areas under investigation

## Risks and safety considerations

## Open questions
